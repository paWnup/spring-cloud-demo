...
* 本地手动启动
    ...
    4. 启动 gateway、service-auth、svca-service、svcb-service
...
```
2. 使用 access token 访问 management-example 接口 f637337a-5318-469e-b727-c528925a070e
```
curl -i -H "Authorization: Bearer eac56504-c4f0-4706-b72e-3dc3acdf45e9" http://localhost:8060/management
```
返回如下数据：
```
svca-service (172.18.0.8:8080)===>name:java
svcb-service (172.18.0.2:8070)===>Say Hello
```
3. 使用 access token 访问 service-example 接口
```
curl -i -H "Authorization: Bearer eac56504-c4f0-4706-b72e-3dc3acdf45e9" http://localhost:8060/service
```
返回如下数据：
```
svcb-service (172.18.0.2:8070)===>Say Hello
